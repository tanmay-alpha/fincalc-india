...
export default async function ResultPage({
    ...
    return (
      <main className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 flex items-center justify-center p-4">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 text-center max-w-sm w-full">
          <div className="text-7xl font-bold text-slate-200 dark:text-slate-800 mb-2">404</div>
          <div className="text-3xl mb-4">🔍</div>
          <h1 className="text-lg font-bold text-slate-900 dark:text-white mb-2">
            Calculation not found
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
            This shared calculation has been revoked or does not exist.
          </p>
          <Link
            href="/"
  ...
  const inputsRecord = (calc.inputs ?? {}) as Record<string, unknown>;
  const outputsRecord = (calc.outputs ?? {}) as Record<string, unknown>;
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 mb-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {Object.entries(inputsRecord).map(([key, val]) => (
              <div
                key={key}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {Object.entries(outputsRecord)
              ...
              .map(([key, val]) => (
                <div
                  key={key}
                  className="bg-blue-50 dark:bg-blue-950/30 rounded-lg p-3"
                >
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 text-center mt-6">
          ⚠️ This is a shared calculation for reference only. Results are
...
export async function generateMetadata({
  ...
  const name = typeNames[calc.type.toLowerCase()] ?? "Financial";
  return {
    ...
    twitter: {
      ...
      title: `Shared ${name} Calculation`,
      description: `View this shared ${name} calculation on FinCalc India.`,
    },
  };
}
